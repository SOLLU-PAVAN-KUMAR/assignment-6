...
```bash
export $(./setOrgEnv.sh airtel | xargs)
```

(Note bash v4 is required for the scripts.)

You will now be able to run the `peer` commands in the context of airtel. If a different command prompt, you can run the same command with jio instead.
