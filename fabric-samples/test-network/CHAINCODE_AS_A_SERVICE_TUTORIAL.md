...
You should be able to see the contract starting in the monitoring window. There will be two containers running, one for jio and one for airtel. The container names contain the organization/peer and the name of the chaincode.
...
```bash
# Environment variables for jio

export CORE_PEER_TLS_ENABLED=true
export CORE_PEER_LOCALMSPID=jioMSP
export CORE_PEER_TLS_ROOTCERT_FILE=${PWD}/organizations/peerOrganizations/jio.example.com/tlsca/tlsca.jio.example.com-cert.pem
...
If you don't have `jq` installed omit `| jq`.  The metadata shows the details of the deployed contract and is JSON, so jq makes it easier to read.  You can repeat the above commands for airtel to confirm that is working.
...
```bash
    docker run --rm -d --name peer0jio_assettx_ccaas  \
                  --network fabric_test \
...
Not starting docker containers; these are the commands we would have run
    docker run --rm -d --name peer0jio_basicj_ccaas                    --network fabric_test                   -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:9999                   -e CHAINCODE_ID=basicj_1.0:59dcd73a14e2db8eab7f7683343ce27ac242b93b4e8075605a460d63a0438405 -e CORE_CHAINCODE_ID_NAME=basicj_1.0:59dcd73a14e2db8eab7f7683343ce27ac242b93b4e8075605a460d63a0438405                     basicj_ccaas_image:latest
```
...
```bash
 docker run --rm -it -p 9229:9229 --name peer0airtel_basic_ccaas --network fabric_test -e DEBUG=true -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:9999 -e CHAINCODE_ID=basic_1.0:7c7dff5cdc43c77ccea028c422b3348c3c1fb5a26ace0077cf3cc627bd355ef0 -e CORE_CHAINCODE_ID_NAME=basic_1.0:7c7dff5cdc43c77ccea028c422b3348c3c1fb5a26ace0077cf3cc627bd355ef0 basic_ccaas_image:latest
```

Java for example, could be started like this

```bash
 docker run --rm -it --name peer0jio_basicj_ccaas -p 8000:8000 --network fabric_test -e DEBUG=true -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:9999 -e CHAINCODE_ID=basicj_1.0:b014a03d8eb1898535e25b4dfeeb3f8244c9f07d91a06aec03e2d19174c45e4f -e CORE_CHAINCODE_ID_NAME=basicj_1.0:b014a03d8e
...
In the peer's environment configuration we then set for jio's peer1

```bash
CHAINCODE_AS_A_SERVICE_BUILDER_CONFIG="{\"peername\":\"jiopeer1\"}"
```

The external builder will then resolve this address to be `jiopeer1_assettransfer_ccaas:9999` for the peer to use.
